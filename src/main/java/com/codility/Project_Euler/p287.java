...
public final class p287 implements EulerSolution {
	 ...
	 * Instead of key and y each in [0, 2^N) for the formula [key - 2^(N-1)]^2 + [y - 2^(N-1)]^2 <= R^2,
	 * we shall consider key and y each in [-(2^(N-1)), 2^(N-1)) for the formula key^2 + y^2 <= R^2.
	 * 
	 ...
	 * 
	 * Within the region, what are the possible values of the left side of the formula, key^2 + y^2?
	 * To minimize or maximize key^2 + y^2, we can min/maximize each of key^2 and y^2 independently.
	 * - To minimize key^2, we minimize |key|. If 0 is in [xStart, xEnd),
	 *   then the minimum |key| is 0, and thus the minimum key^2 is 0.
	 *   Otherwise, either all possible key values are negative or all
	 *   are positive, so the minimum |key| is min(|xStart|, |xEnd-1|).
	 * - To maximize key^2, we maximize |key|. This simply equals max(|xStart|, |xEnd-1|).
	 ...
	 *   key^2 + y^2 <= R^2, hence the entire region is black.
	 * - Similarly, if minR^2 > R^2, then all points in the region
	 *   satisfy key^2 + y^2 > R^2, hence the entire region is white.
