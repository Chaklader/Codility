...
An empty tree is represented by an empty pointer (denoted by null). A non-empty tree is represented by a pointer to an object representing its root. The attribute key holds the integer contained in the root, whereas attributes left and right hold the left and right subtrees of the binary tree, respectively.
